...
def readArff(filename):
    ...
    for i in range(len(trainset)):
        ...
        for j in range(len(temp)):
            ...
            if type(temp[j])==ty:
                ...
                s.append(temp[j])
        train.append(s)     
    use=DataFrame(train,columns=meta.names())
    feature=OrderedDict()
    for i in meta.names():
        if meta[i][1]==None:
            feature[i]=[]
    ...
    return[use,meta.names(),feature]

def InfoGain_nomial(data,feature,filenames):#???
    """
    to calculate the nomial feature Information Gain
    """
    #labels=readArff("credit_train.arff")[2]
    labels=readArff(filenames)[2]#???
...
def FindBestSplit(train_data,all_feature,filenames):#???
    answer=float("-inf");list_node=[]
    for i in all_feature:
        if train_data[i].dtypes=="O":
            IG=InfoGain_nomial(train_data,i,filenames)[0]
        else:
    ...
    if answer<0:
        ...
        if train_data[best_node].dtypes=="O":
            list_node.append(best_node);list_node.append(InfoGain_nomial(train_data,best_node,filenames)[1]);list_node.append(zero)#???
        else:
...
def MakeSubetree(train_data,m,filenames,zero=False,tree_model=[],d=OrderedDict(),best_node=None):#???
    if train_data.shape[0]<m or len(train_data["class"].unique())==1 or zero:
        ...
        ans=FindBestSplit(train_data,all_feature,filenames)
        ...
        if train_data[best_node].dtypes=="O":
            ...
            for l in labels:
                ...
                all_feature=DetermineCandidateSplits(train_data,best_node)
                d=MakeSubetree(train_data,m,filenames,zero,tree_model,d,best_node)[-1]#???
        else:
            p=ans[1]
            ...
            for ope in D.keys():
                ...
                d=MakeSubetree(train_data,m,filenames,zero,tree_model,d)[-1]#???
    d=OrderedDict()#???     
    return [tree_model,d]
...
def final_predit(train_data,test_data,m):
    ...
    tree_model_final=MakeSubetree(tree,m,filenames,tree_model=[],d=OrderedDict())[0]#???
